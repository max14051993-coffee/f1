...
import { filterEventsByVisibility, localizeEvent, type LocalizedScheduleEvent } from '../lib/schedule';
import type { ScheduleEvent } from '../lib/ics';
...
export default function Home({ initialEvents }: { initialEvents: ScheduleEvent[] }) {
  const { theme, toggleTheme } = useThemePreference();
  const [language, setLanguage] = useLanguagePreference();
  const { events, isLoading, isError, errorMessage, reload } = useScheduleLoader(initialEvents);
  ...
  // `now === null` until mounted: prerender and first hydration render stay
  // deterministic (UTC + scheduled status), so the build-time HTML matches the
  // client markup exactly. After mount the live clock takes over.
  const [now, setNow] = useState<DateTime | null>(null);
  useEffect(() => {
    // Deferred so the clock state update never runs synchronously inside the
    // effect body (react-hooks/set-state-in-effect).
    void Promise.resolve().then(() => setNow(DateTime.local()));
  ...
  const localizedEvents = useMemo(() => {
    if (now === null) {
      return filtered.map(event => ({
        event,
        localStart: DateTime.fromISO(event.startsAtUtc, { zone: 'utc' }).setLocale(locale),
        localEnd: event.endsAtUtc
          ? DateTime.fromISO(event.endsAtUtc, { zone: 'utc' }).setLocale(locale)
          : null,
        startRelative: null,
        finishRelative: null,
    ...
    const nowLocal = now.setLocale(locale);
    return filtered.map(event => localizeEvent(event, now.zoneName ?? 'utc', locale, nowLocal));
  }, [filtered, locale, now]);
